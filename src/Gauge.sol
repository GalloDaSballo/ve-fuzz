...
contract Gauge is IGauge {
    ...
    constructor(address _stake, address _external_bribe, address  __ve, address _voter, address _oFlow, address _gaugeFactory, bool _forPair, address[] memory _allowedRewardTokens) {
        ...
        isForPair = _forPair;
        flow = IVotingEscrow(_ve).baseToken(); /// @audit VE changes!!!
    ...
    */
    ...
    function getReward(address account, address[] memory tokens) external lock {
        require(msg.sender == account || msg.sender == voter, "req");
        _unlocked = 1;
        IVoter(voter).distribute(address(this)); /// @audit Tokens with Hooks can reEnter here
        _unlocked = 2;
        ...
        for (uint i = 0; i < tokens.length; i++) {
            ...
            if (_reward > 0) {
                if (tokens[i] == flow) { /// @audit WHY? (INvariant to see if this can happen as well)
    ...
    function setOFlow(address _oFlow) external {
        ...
        oFlow = _oFlow;
        _safeApprove(flow, _oFlow, type(uint256).max); /// @audit Allowance not cleared
