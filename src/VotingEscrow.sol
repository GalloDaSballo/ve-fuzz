...
import {IVeArtProxy} from "src/interfaces/IVeArtProxy.sol";
...
contract VotingEscrow is IERC721, IERC721Metadata, IVotes {
    ...
    /// @param lp_token_addr `LP FLOW` token address
    /// @param base_token_addr `FLOW` token address
    ...
    function _isApprovedOrOwner(address _spender, uint _tokenId) internal returns (bool) {
        max_lock(_tokenId); /// @audit This is a DOS waiting to happen
    ...
    /// @notice remove tokens from the nft so they can be added to difrent one
    ...
    function _remove_from(
        ...
        emit Remove(from, _tokenId, _value, _locked.end, block.timestamp);
    ...
    function _deposit_for(
        ...
        if (_value != 0 && deposit_type != DepositType.MERGE_TYPE && deposit_type != DepositType.SPLIT_TYPE) {
            require(IERC20(lpToken).transferFrom(from, address(this), _value));
        }
    ...
    function withdraw(uint _tokenId) external nonreentrant {
        ...
        require(IERC20(lpToken).transfer(msg.sender, value));
    ...
    function detach(uint _tokenId) external {
        require(msg.sender == voter);
        attachments[_tokenId] = attachments[_tokenId] - 1; /// @audit Arbitrary sub
    }
     ...
     * @param amount  balance of new created nft
     */
